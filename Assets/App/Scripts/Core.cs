...
namespace NRatel.TextureUnpacker
{
    ...
    {
        //仅从大图中裁剪出小图
        public static void Split(Texture2D bigTexture, Frame frame)
        {
            ...
            Save(null, frame.textureName, bytes);
            Texture2D.DestroyImmediate(destTexture);
            //estTexture = null;
        }

        public static void SplitAll(Texture2D bigTexture, Plist plist)
        {
            foreach (var frame in plist.frames)
            {
                Split(bigTexture, frame);
            }
        }
        ...
        public static void Restore(Texture2D bigTexture, Frame frame)
        {
            ...
            destTexture.Apply();
            byte[] bytes = destTexture.EncodeToPNG();
            Save("__RestoredPngs__", frame.textureName, bytes);
            Texture2D.Destroy(destTexture);
            destTexture = null;
        }

        public static void RestoreAll(Texture2D bigTexture, IEnumerable<Frame> frames)
        {
            foreach (var frame in frames)
            {
                Restore(bigTexture, frame);
            }
        }

        private static void Save(string subDir, string textureName, byte[] bytes)
        {
            string dir = (subDir != null) ? Path.Combine(App.GetSaveDir(), subDir) : App.GetSaveDir();
            ...
            using (var file = File.Open(Path.Combine(dir, textureName), FileMode.Create))
            {
